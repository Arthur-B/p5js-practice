...
function setup() {
  ...
  // Setup the colors
  c_frame = [
    color('white'), // Background, Bone White
    color('black'), // Text, Ebony
  ];
...
function draw() {
  ...
  push();
  stroke('blue');
  recursiveHilbert(-10, -10, content_size.x * 0.9, 5);
  pop();

  push();
  stroke('green');
  recursiveHilbert(0, 0, content_size.x * 0.9, 5);
  pop();
...
recursiveHilbert =
  (cx, cy, size, order) => recursiveHilbertHelper(cx, cy, size, order, 0);

recursiveHilbertHelper = (cx, cy, size, order, rot) => {
  push();
  translate(cx, cy);
  rotate(rot * PI / 2);
  if (order > 1) {
